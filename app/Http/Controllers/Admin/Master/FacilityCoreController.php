...
class FacilityCoreController extends Controller
{
    protected $prefixRoute = 'admin.master.facilitycore.';
    public function index()
    {
        $data['data'] = FacilityCore::with('pageDetail')->get();
         $data['prefixRoute'] = $this->prefixRoute;
        $data['cbo']['pageDetail'] = PageDetail::select('id', 'title')->get();
        $data['title'] = 'Service Core';
    ...
    { 
        ...
        if($data->id == 0){
            $sort = FacilityCore::where('sort',$data->sort)->count();
        } else {
            $sort = FacilityCore::where('sort',$data->sort)->where('id','!=',$data->id)->count();
        }
        ...
        if($sort != 0){
            ...
            if($data->id == 0){
                FacilityCore::create($dataSave);
            } else {
                FacilityCore::where('id', $data->id)->update($dataSave);
            }
    ...
    {
        FacilityCore::findOrFail($id)->delete();
        return redirect()->route($this->prefixRoute.'index')->withSuccess('Data Berhasil dihapus!');
    }

    public function multi_delete(Request $req)
    {
        foreach (FacilityCore::whereIn('id', $req->id)->get() as $row) {
            FacilityCore::findOrFail($row['id'])->delete();
        }
        return redirect()->route($this->prefixRoute.'index')->withSuccess('Data Berhasil dihapus!');
    }
    ...
    {
        FacilityCore::where('id', $id)->update(['status'=>$request->sts]);
        return response()->json(['status' => 'success', 'message' => 'Status Berhasil diubah']);
    }

    public function edit($id)
    {
        $data = FacilityCore::where('id',$id)->get();
