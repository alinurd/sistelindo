<?php

namespace App\Http\Controllers\Admin\Master;

use App\Http\Controllers\Controller;
use App\Models\Master\Happycustomer;
use App\Models\Master\PageDetail;
use Illuminate\Http\Request;

class HappycustomerController extends Controller
{
    protected $prefixRoute = 'admin.master.happycustomer.';

    public function index()
    {
        $data['data'] = Happycustomer::with('pageDetail')->get();
        $data['prefixRoute'] = $this->prefixRoute;
        $data['cbo']['pageDetail'] = PageDetail::select('id', 'title')->where('status',1)->get();
        $data['title'] = 'Happy Customer';
        return view("admin.master.happycustomer.index", $data);
    }

    public function create(Request $request)
    {
        $data = $request->only(['id', 'sort', 'status', 'title', 'description', 'image','rating']);
        $data = (object) $data;

        if($data->id == 0){
            $sort = Happycustomer::where('sort',$data->sort)->count();
        } else {
            $sort = Happycustomer::where('sort',$data->sort)->where('id','!=',$data->id)->count();
        }

        if($sort != 0){
            return response()->json(['status'=> 'error', 'message' => 'Sort sudah dipakai']);
        } else {
            if(!empty($data->image)) {
                $data->image = str_replace(url('/').'/', '', $data->image);
            }

            $dataSave = collect($data)->except(['id'])->toArray();
            if($data->id == 0){
                Happycustomer::create($dataSave);
            } else {
                Happycustomer::where('id', $data->id)->update($dataSave);
            }
            return response()->json(['status'=>'success', 'message' => 'Data Berhasil disimpan']);
        }
    }

    public function delete($id)
    {
        Happycustomer::findOrFail($id)->delete();
        return redirect()->route($this->prefixRoute.'index')->withSuccess('Data Berhasil dihapus!');
    }

    public function multi_delete(Request $req)
    {
        foreach (Happycustomer::whereIn('id', $req->id)->get() as $row) {
            Happycustomer::findOrFail($row['id'])->delete();
        }
        return redirect()->route($this->prefixRoute.'index')->withSuccess('Data Berhasil dihapus!');
    }

    public function editstatus($id, Request $request)
    {
        Happycustomer::where('id', $id)->update(['status'=>$request->sts]);
        return response()->json(['status' => 'success', 'message' => 'Status Berhasil diubah']);
    }

    public function edit($id)
    {
        $data = Happycustomer::where('id',$id)->get();
        return response()->json(['status'=>'success','data'=>$data]);
    }
}